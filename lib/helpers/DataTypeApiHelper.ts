...
export class DataTypeApiHelper extends TreeApiHelper {
  protected resourcePath = 'data-type';
  protected treePath = 'tree/data-type';
  ...
  async createDataType(name: string, editorAlias: string, editorUiAlias: string, values: { alias: string; value: string; }[], parentId?: string, id?: string) {
    const dataType = {
      "name": name,
  ...
  async update(id: string, dataType: any): Promise<any> {
  ...
  async createFolder(name: string, parentId?: string, id?: string): Promise<string> {
    const folderData = {
      "name": name,
      "id": id,
      "parent": parentId ? {"id": parentId} : null
    ...
    return response.headers().location.split("v1/data-type/folder/").pop() as string;
  }
