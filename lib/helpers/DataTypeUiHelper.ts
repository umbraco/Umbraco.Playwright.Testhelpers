...
export class DataTypeUiHelper extends UiBaseLocators {
  ...
  async selectAPropertyEditor(propertyName: string, filterKeyword?: string) {
    await this.enterText(this.typeToFilterTxt, filterKeyword ? filterKeyword : propertyName, {clearFirst: false});
  ...
  async clickRemoveBlockWithName(name: string) {
    ...
    await this.waitForVisible(blockWithNameLocator);
  ...
  async doesAmountContainErrorMessageWithText(errorMessage: string) {
    return await this.isVisible(this.page.getByText(errorMessage));
  }
  ...
  async enterBlockLabelText(label: string) {
    ...
    await this.enterText(this.labelTextTxt, label, {clearFirst: false});
  }

  async removeBlockLabelText() {
    await this.clearText(this.labelTextTxt);
  }
  ...
  async isElementWorkspaceOpenInBlock(elementTypeName: string) {
    return await this.isVisible(this.documentTypeWorkspace.filter({hasText: elementTypeName}));
  }
  ...
  async chooseBlockThumbnailWithPath(mediaPath: string) {
    ...
    for (let i = 0; i < mediaItems.length; i++) {
      if (i === mediaItems.length - 1) {
        ...
        await this.click(this.page.locator('uui-menu-item[label="' + mediaItems[i] + '"] #caret-button'));
      }
    }
    await this.clickChooseModalButton();
  }
  ...
  async enterGridColumns(value: number) {
    ...
    await this.enterText(this.gridColumnsTxt, value.toString());
  }
  ...
  async enterMinRowSpan(value: number) {
    ...
    await this.enterText(this.availableRowSpansLowValueTxt, value.toString());
  }
  ...
  async enterMaxRowSpan(value: number) {
    ...
    await this.enterText(this.availableRowSpansHighValueTxt, value.toString());
  }
  ...
  async enterGridColumnsForArea(value: number) {
    ...
    await this.enterText(this.areaGridColumnsTxt, value.toString());
  }
  ...
  async enterCreateButtonLabelInArea(value: string) {
    ...
    await this.enterText(this.createLabelTxt.nth(1), value);
  }
  ...
  async enterMinAllowedInArea(value: number) {
    ...
    await this.enterText(this.minAllowedTxt, value.toString());
  }
  ...
  async enterMaxAllowedInArea(value: number) {
    ...
    await this.enterText(this.maxAllowedTxt, value.toString());
  }
  ...
  async getLinkWithName(name: string) {
    await this.waitForVisible(this.page.getByRole('link', {name: name}));
    return this.page.getByRole('link', {name: name});
  }

  async getAddButtonInGroupWithName(name: string) {
    await this.waitForVisible(this.page.locator('.group').filter({hasText: name}).locator('#add-button'));
  ...
  async clickRemoveStylesheetButton(stylesheetName: string) {
    await this.click(this.page.locator('[name="' + stylesheetName + '"]').getByLabel('Remove'));
  }
  ...
  async deleteToolbarGroup(groupIndex: number, rowIndex: number = 0) {
    ...
    await this.waitForVisible(groupButton);
  ...
  async doesSettingHaveValue(settings) {
    for (let index = 0; index < Object.keys(settings).length; index++) {
      ...
      await this.hasText(this.propertyEditorConfigItems.nth(index).locator('#headerColumn #label'), label);
      if (description !== '')
        await this.hasText(this.propertyEditorConfigItems.nth(index).locator('#description'), description);
    }
  }
  ...
  async isExtensionItemChecked(itemName: string, isChecked: boolean = true) {
    if (isChecked) {
      await this.waitForChecked(this.tiptapExtensionsConfiguration.locator('uui-checkbox[label="' + itemName + '"] input'));
    } else {
      await this.waitForUnchecked(this.tiptapExtensionsConfiguration.locator('uui-checkbox[label="' + itemName + '"] input'));
    }
  }
