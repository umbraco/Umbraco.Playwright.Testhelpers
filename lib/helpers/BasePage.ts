...
export class BasePage {
   ...
   */
  async isEditable(locator: Locator, editable: boolean = true, timeout?: number): Promise<void> {
    await expect(locator).toBeEditable({editable, timeout: timeout ?? ConstantHelper.timeout.medium});
  }
   ...
   * @param timeout - Optional timeout in milliseconds
   */
  async doesNotHaveAttribute(locator: Locator, name: string, value: string, timeout?: number): Promise<void> {
    await expect(locator).not.toHaveAttribute(name, value, {timeout: timeout ?? ConstantHelper.timeout.medium});
  }
